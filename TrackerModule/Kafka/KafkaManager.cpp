...
void KafkaManager::loadConfigurationGroup(rd_kafka_conf_t * conf, GKeyFile * key_file, const char * group) {
    ...
    while (* ptr) {
        ...
        if (rd_kafka_conf_set(conf, key, value, errstr, sizeof(errstr)) != RD_KAFKA_CONF_OK) exit(-4);
        ptr++;
    }
}
...
void KafkaManager::setupProducer (const char* configFile) {
    char errorString[512];
    const char * configurationFile = configFile;
