...
#define OPENPOSE_BODY_KEYPOINTS 25
#define JSON_FILE_PATH "skeletonsPoints3D.json"
...
private:
     ...
     */
    void deprojectSkeletonPoints3D  (RealSenseCamera* camera);
    /**
...
public:
     ...
     */
    Skeleton                                        (cv::Mat & rgbImage, cv::Mat & dImage, Json::Value skeletonData);
    /**
     ...
     */
    void generateSkeleton                           (RealSenseCamera* camera, CoordinateMappingManager* mapping);
    /**
...
namespace JSONOutput {
    Json::Value makeOutputString (std::vector <Point3D *> skeletonPoints3D, std::vector <bool> bodyKeyPointsMap, unsigned int frameID, unsigned int personID);
    bool loadJSON (std::string filePathJSON, Json::Value & currentJSON);
    void saveJSON (std::string JSONdata, std::string filePath);
    ...
    void createJSON (Json::Value & people, unsigned int nFrame, cv::Mat & colorImage, cv::Mat & distanceImage, RealSenseCamera* camera, CoordinateMappingManager* mapping, std::string thingID = "thing", const float skeletonThreshold = 40, std::string outputFolder = "feature-json");
};
